...
{
    enum TimeOfDay timeOfDay = OW_TIME_OF_DAY_ENCOUNTERS ? GetTimeOfDay(MODE_GENERIC) : TIME_OF_DAY_DEFAULT;
...
{
    timeOfDay = timeOfDay == TIME_NIGHT ? TIME_MIDNIGHT : timeOfDay + 1; // Zatsu TODO
    return GenConfigTimeOfDay(timeOfDay);
}

enum TimeOfDay TryDecrementTimeOfDay(enum TimeOfDay timeOfDay)
{
    timeOfDay = timeOfDay == TIME_MIDNIGHT ? TIME_NIGHT : timeOfDay - 1; // Zatsu TODO
    return GenConfigTimeOfDay(timeOfDay);
}
